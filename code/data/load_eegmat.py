...
def load_eegmat(
    ...
    """
    Load and epoch the full EEGMAT dataset.

    Task features are text embeddings from natural-language descriptions
    of each condition (rest / arithmetic × early / mid / late × performance).

    Returns dict with keys: eeg, labels, subject_ids, condition_ids, task_features
    """
    if mne is None:
        raise ImportError("mne is required — pip install mne")

    # ── pre-compute text embeddings for all conditions ──
    text_emb = build_text_embeddings(lang="en", cache_dir=cache_dir)
    ...
    if not edf_files:
        raise FileNotFoundError(f"No Subject*_*.edf files found in {root_dir}")
        ...
        for suffix, label in [(1, LABEL_REST), (2, LABEL_ARITH)]:
            ...
            if target_sfreq and abs(sfreq - target_sfreq) > 1:
                raw_tmp = mne.io.RawArray(
                    data, mne.create_info(ch_names, sfreq, ch_types="eeg"),
                    verbose=False,
                )
